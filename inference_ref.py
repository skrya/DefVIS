...
def process_exp(exp, tokenizer) :
    ...
    return  torch.cat(exps_input_ids, dim=0), torch.cat(exps_attn_masks, dim=0)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for id_ in videos.keys():
            ...
            for each_exp_item in expressions_.keys() :

                exp = expressions_[each_exp_item]["exp"]
                word_ids, word_attn_mask = process_exp(exp, tokenizer)
                #print(f'word_ids {word_ids.shape}') # word_ids [1,20]
                # inference time is calculated for this operation
                outputs = model(img, word_ids.cuda(), word_attn_mask.cuda())
                ...
                for m in range(num_ins):
                    ...
                    if score > max_score :
                        max_score = score
                        max_m = m
                score = pred_scores[:,max_m].mean()
                #category_id = pred_logits[:,m][pred_scores[:,m].argmax()]
                category_id = np.argmax(np.bincount(pred_logits[:,max_m]))
                instance = {'video_id':id_, 'score':float(score), 'category_id':int(category_id)}
                segmentation = []
                for n in range(length):
                    if pred_scores[n,max_m]<0.001:
                        segmentation.append(None)
                    else:
                        mask = (pred_masks[n,max_m]).astype(np.uint8) 
